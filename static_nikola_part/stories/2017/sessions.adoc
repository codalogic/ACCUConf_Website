...
This is a fun session based on the popular UK game show _Countdown_ [1] and its parody, _8 out of 10 cats
does Countdown_ [2].

In each round, instead of picking vowels and consonants the teams take it in turns to pick ~6 {cpp} tokens
from 5 categories: keywords, identifiers, operators, punctuators, and literals!

The aim of each round is to write the smallest program containing all the tokens. After we stop and review
the submissions, points will be awarded!  There will be winners and losers, but most of all, there will be
learning and fun.

We may also introduce some bonus rounds including surprise other languages!  We will be using the online
coding practice site http://cyber-dojo.org to facilitate the competition.

Numbers are limited: at most 8 teams, two people per team.

But, if you aren't in an official team, you can still be part of the audience, where you are invited to
"play along at home".
...
[[XGradleBuildinganddeliveringSoftwareinamodernworld]]
== Gradle: Building and delivering Software in a modern world
=== link:presenters.html#XBernhard_Merkle[Bernhard Merkle]

Modern software projects usually contain a mixture of languages and tools. Delivering software becomes a challenge as different language aware tools (e.g. ant/maven for Java, (c)make for {cpp}) 
are used for managing the build pipeline. And invoking the compiler + linker is only 20% of the story.
Managing a build also involves clean dependency management, multiple configurations and variants, cross-plattform builds (e.g. {cpp}), documentation, test, deployments, devops and more. 
...
I will present the examples live (expect to see an editor + code and not just slides ;-) and also talk about strategies for migrating from old build systems towards Gradle.
We will show how we migrated a large Java and {cpp} system which suffered from ant-hell and IDE specific project files towards build happiness with Gradle.
...
[[XLiberatingWirelessSensorData]]
== Liberating Wireless Sensor Data
...
This presentation will be a case study of Paxport's initial adoption of machine learning where we will go over our tools selection of Python with Jupyter as the main framework and its importance when dealing with uncertainty and the need for quick and reliable experimentation.

More specifically we will be looking at a recommendation system for accommodation bookings based on Collaborative Filtering approaches by going over different stages and pointing certain aspects such as important libraries used (Pandas, Scikit-learn, etc.), data preparation/analysis, performance/scalability needs, implementation/deployment choices and results evaluation.
...
=== link:presenters.html#XAlfred_Bratterud[Alfred Bratterud]
...
real-world {cpp} code. I'll pick which one (or more) of those topics to present sometime in March.
...
[[Xlintingwithclangandclangtidy]]
== linting with clang and clang-tidy
=== link:presenters.html#XBernhard_Merkle[Bernhard Merkle]

In this session we will use clang's features to find bugs at compile time (via static analysis) and runtime (via sanitizers). First we will look how clang is able to detect common C/{cpp} traps and pitfalls via static analysis. We will write our own code checker to implement a common {cpp} guideline. Second, we will demonstrate sanitizers which enable code instrumentation and detection of problems which happen at runtime.
